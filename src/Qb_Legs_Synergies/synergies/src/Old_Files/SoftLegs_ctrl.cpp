...
//if speed variation is less than 0.01 m/s ingnore it
...
int main(int argc, char **argv)
{
	...
	// Import Synergies
	// Eigen::MatrixXd PCs(r_PCs,c_PCs); 
	// Eigen::MatrixXd Mus(r_Mus,c_Mus); 
	...
	while (ros::ok())
	{
		...
		else
		{
			...
			if (speed != Current_Speed && one_time == true && fabs(Current_Speed - speed) >= Var_speed_threshold)
			{
				...
				Old_Traj = New_Traj;
				New_Traj = From_Syn_To_Traj(Current_Speed, foot_h, foot_l, PCs, Mus);

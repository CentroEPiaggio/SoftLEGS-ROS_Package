...
// 	std::vector<int> cube_id = {4,1,2,3,5,6};
...
int main(int argc, char **argv)
{
	...
	stiffness.resize(n_cube);
	stiffness_val.resize(n_cube);
...
//     std::cout<<" "<<stiffness[i]<<std::endl;
	...
	int *volt_current_measurement=new int;
	
	bool flag_retrieve_currents=true;
	bool flag_retrieve_currents_and_voltage=true;
	...
	ros::Publisher pub_voltage_qb_legs = n_.advertise<std_msgs::Float64MultiArray>("/qb_legs_voltage",10);
	
	geometry_msgs::Pose qb_legs_msg;
	std_msgs::Float64MultiArray qb_legs_curr_msg;
	std_msgs::Float64MultiArray qb_legs_volt_msg;
	...
	qb_legs_curr_msg.data.resize(2*n_cube);
	qb_legs_volt_msg.data.resize(n_cube);
...
// 			  int ris=commGetCurrents(&comm_settings_t, id, current_measurements); //here get current motor measurements
// 			  usleep(100);
// 			  if(ris==0){
...
// 			}
			...
			if(flag_retrieve_currents_and_voltage){
			  ris=commGetCurrents(&comm_settings_t, id, current_measurements); //here get current motor measurements
			  ris2=commGetTension(&comm_settings_t, id, volt_current_measurement);
// 			  std::cout<<ris<<" "<<ris2<<std::endl;
			  usleep(100);
			  if(ris==0){
			    qb_legs_curr_msg.data.at(count++) = current_measurements[0];
			    qb_legs_curr_msg.data.at(count++) = current_measurements[1];
			 ...
			 if(ris2==0){
// 			   std::cout<<*volt_current_measurement<<std::endl;
			    qb_legs_volt_msg.data.at(count_v++) = *volt_current_measurement;
			  }else{count_v++;}
			}
...
// 		std::cout<<" meas "<<tick2rad(measurements[0])<<" "<<tick2rad(measurements[1])<<" "<<tick2rad(measurements[2])<<std::endl;
		...
		pub_voltage_qb_legs.publish(qb_legs_volt_msg);

...
#include <qb_ctrl_voltage/qbmove_communications.h>
...
//     std::cout<<" "<<stiffness[i]<<std::endl;
	...
	short int *volt_current_measurement=new short int;
...
// 	ros::Publisher  pub_measure_qb_legs = n_.advertise<geometry_msgs::Pose>("/qb_legs_measures", 10);
	...
	ros::Publisher pub_tension_qb_legs = n_.advertise<std_msgs::Float64MultiArray>("/qb_legs_voltage",10);
	...
	{	int count=0;
		...
		for(auto id:cube_id)
		{
			...
			if(flag_retrieve_currents_and_voltage){
			  ...
			  int ris2=commGetTension(&comm_settings_t, id, volt_current_measurement);
...
// 		pub_measure_qb_legs.publish(qb_legs_msg);
		...
		pub_tension_qb_legs.publish(qb_legs_volt_msg);

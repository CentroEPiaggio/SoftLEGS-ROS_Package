...
{	
  ...
  //------------------------
  pub_flag_gui=n.advertise<std_msgs::Bool>("/optimal_gui/flag_opt",10);
  std_msgs::Bool msg_b;
  msg_b.data=true;
...
//   pub_activate_quant=n.advertise<std_msgs::Bool>("/activate_quant",10);
  ...
  bias_pub=n.advertise<std_msgs::Float64MultiArray>("/optimal_walk/optimal_bias",10);
  bias_sub=n.subscribe("/optimal_walk/bias_echo",10,&qt_example::Retrieve_Bias,this);
  
  curr_sub=n.subscribe("/qb_legs_currents",10,&qt_example::Retrieve_Current,this);
  volt_sub=n.subscribe("/qb_legs_voltage",10,&qt_example::Retrieve_Voltage,this);
  
//   bias_stop_pub=n.advertise<optimal_walk_gui::bias>("/synergies_bias_stop",10);
//   BT_pub=n.advertise<std_msgs::Float32>("/update_BT",10);
//   AT_pub=n.advertise<optimal_walk_gui::gain_bias_err>("/gain_bias_trunk_ankles",10);
...
//   pub_GG=n.advertise<optimal_walk_gui::GG_msg>("/update_G",10);
//   pub_ddes=n.advertise<std_msgs::Float32>("/update_d_des",10);
  pub_stiff=n.advertise<geometry_msgs::Pose>("/qb_legs_stiffness",10);
  pub_des_opt=n.advertise<std_msgs::Float64>("/optimal_walk/des_opt",10);
  
  pub_start=n.advertise<std_msgs::Float64>("/optimal_walk/start",10);
    pub_stop=n.advertise<std_msgs::Float64>("/optimal_walk/stop",10);

    QImage myImage;
	myImage.load(QString::fromStdString(ros::package::getPath("optimal_walk_gui")+"/NO_SoftLegs_GUI.png"));
...
//     myLabel->setPixmap(pix);
  ...
  QGridLayout * grid_3=new QGridLayout;
  
  grid->addLayout(grid_1,0,0);
  grid->addLayout(grid_2,1,0);
  grid->addLayout(grid_3,2,0);
    ...
    count=0;
    count_r=0;
grid_3->addWidget(createCurrentVoltageGroup(),0,0);
  
grid_2->addWidget(createDistanceGroup(),count_r,count++);
// grid_2->addWidget(createDistanceGroup(),count_r,count++);

//   grid_2->addWidget(createCurrentVoltageGroup(),++count_r,--count);
// count_r--;
// count++;
...
QGroupBox* qt_example::createOptGroup(){
  QGroupBox* groupBox = new QGroupBox(tr("Optimal Walk"));
...
//   OptList= new QListWidget();
  ...
  OptList=new QListWidget();
    ...
    button_start_Opt.setText("Start OPT");
    button_stop_Opt.setText("Stop OPT");
...
//     groupBox->setLayout(vbox);
    ...
    button_3.setText("Launch NO");
    button_4.setText("Kill NO");
...
//     biasLineEdit_write->setInputMask("xxx,xxx,xxx,xxx,xxx,xxx");
    biasLineEdit_write->setText("-15,0,25,-3,0,-20"); //-1,0,15,-3,0,16
...
QGroupBox * qt_example::createCurrentVoltageGroup()
{
  QGroupBox *groupBox= new QGroupBox(tr("Curr and Volt"));
  ...
  m_TableHeader<<"H_1"<<"K_2"<<"A_3"<<"H_4"<<"K_5"<<"A_6";
  m_pTableWidget->setHorizontalHeaderLabels(m_TableHeader);
  m_TableHeader_2<<"Curr "<<"Volt ";
   m_pTableWidget->setVerticalHeaderLabels(m_TableHeader_2);
  m_pTableWidget->setShowGrid(true);
 
  
    // set the default size, here i've set it to 20px by 20x
  m_pTableWidget->horizontalHeader()->setDefaultSectionSize(70);
  ...
  m_pTableWidget->verticalHeader()->setResizeMode(QHeaderView::Fixed);
  ...
  for(int i=0;i<2;i++)
  {
    for(int j=0; j<6; j++)
    {
      ...
      m_pTableWidget->setItem(i,j,item);
    }
  }
  
  QGridLayout * n_grid=new QGridLayout;
  
  n_grid->addWidget(m_pTableWidget,0,0);
  m_pTableWidget->setMaximumWidth(70*7-20);
  m_pTableWidget->setMaximumHeight(20*3+10);
//   n_grid->setColumnFixedWidth( 0, 70*7 );
  groupBox->setLayout(n_grid);

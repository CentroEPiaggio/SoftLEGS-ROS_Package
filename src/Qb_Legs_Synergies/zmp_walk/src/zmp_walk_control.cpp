...
#include <zmp_walk/Optimal_Dataset_ZMP.h>
#include <zmp_walk/num_sim_srv.h>
#include <zmp_walk/sim_start_srv.h>
#include <zmp_walk/sim_stop_srv.h>

#include <zmp_walk/spline.h>
#include <zmp_walk/Supporting_functions.h>
#include <zmp_walk/dataset_name.h>
...
int main(int argc,char** argv){
    ...
    ros::init(argc,argv,"zmp_walk");
    ros::NodeHandle n;
    
    bool flag_pub_traj=true;
    bool flag_concatenate_started=false;

    ros::ServiceServer srv_set_Sim = n.advertiseService("zmp_walk/Set_Sim", &setSimCallback); 
    ros::ServiceServer srv_sim_start = n.advertiseService("zmp_walk/Sim_Start", &setSimStartCallback);
    ros::ServiceServer srv_sim_stop = n.advertiseService("zmp_walk/Sim_Stop", &setSimStopCallback);
    
    ros::Subscriber sub_bias=n.subscribe("zmp_walk/optimal_bias",1,&BiasCallback);
    ros::Subscriber sub_start=n.subscribe("zmp_walk/start",1,&WarmStartCallback);
    ros::Subscriber sub_stop=n.subscribe("zmp_walk/stop",1,&WarmStopCallback);
    ros::Publisher pub_traj= n.advertise<std_msgs::Float64MultiArray>("zmp_walk/trajectories",10);
    ros::Publisher qb_legs_pub=n.advertise<geometry_msgs::Pose>("/qb_legs_trajectories",10);
    ros::Publisher pub_opt_bias_echo=n.advertise<std_msgs::Float64MultiArray>("zmp_walk/bias_echo",10);
    ros::Subscriber sub_des_opt=n.subscribe("zmp_walk/des_opt",1,&DesOptCallback);
    
    ros::Publisher pub_dataset_name=n.advertise<zmp_walk::dataset_name>("zmp_walk/dataset_name",10);
    ...
    std::cout<<sim_name[i]<<std::endl;
        ...
        zmp_walk::dataset_name opt_sims;
...
//     std::cout<<((traj_p)[0]).rows()<<" "<<((traj_p)[0]).cols()<<std::endl;
//     for(int i=0;i<=((traj_p)[0]).size()-1;i++){
//     std::cout<<(traj_p)[0](0,12)<<std::endl;
//     }
//     
// //     Current_speed---> Des_Optimal_sim
// //     speed--->Curr_Optimal_sim
// 	
// 
    double pub_Ts=0.004;
...
while(ros::ok()){
  ...
  if(Des_Optimal_sim<=0){Des_Optimal_sim=1;}
  if(Curr_Optimal_sim<=0){Curr_Optimal_sim=1;}
...
//     std::cout<<__func__<<" 263 "<<std::endl;		
		traj_d[i]=Output_Traj(t_index,i);
//     std::cout<<"Out "<<Output_Traj.rows()<<" "<<Output_Traj.cols()<<std::endl;
//     traj_d[i]=Output_Traj(i,t_index);
...
for(int i=0;i<6;i++){sim_traj_msg.data.at(i)=traj_d[i];}
...
// 	bias_msg_echo.bias[id-1]=bias[id-1];
	...
	if(id==1){qb_msg.position.x=qb_val;}
	if(id==2){qb_msg.position.y=qb_val;}
	if(id==3){qb_msg.position.z=qb_val;}
	if(id==4){qb_msg.orientation.x=qb_val;}
	if(id==5){qb_msg.orientation.y=qb_val;}
	if(id==6){qb_msg.orientation.z=qb_val;}

...
static const char *optString = "g:i:kadmMsu:o:prltvhqw:b?W:";
...
//=======================================================     processing options
    ...
    while ((option = getopt_long( argc, argv, optString, longOpts, &longIndex )) != -1)
    {
      switch (option)
        ...
        case 'R':
            sscanf(optarg,"%d", &aux_int);
            global_args.BaudRate = (short int) aux_int;

            global_args.flag_set_baudrate = 1;
            break;
        case 'W':
            sscanf(optarg,"%d", &aux_int);
            global_args.WDT = (short int) aux_int;
...
//=================================================================     watchdog
    ...
    if(global_args.flag_set_watchdog)
    {
        ...
        if (global_args.WDT > MAX_WATCHDOG_TIME){
            ...
            if (global_args.WDT <= 0){
                puts("Watchdog DISABLED.\n");
                global_args.WDT = 0;
            }

        commSetWatchDog(&comm_settings_t, global_args.device_id, global_args.WDT);
...
//============================================================     baudrate
    if (global_args.flag_set_baudrate){
        ...
        if (((int) global_args.BaudRate == BAUD_RATE_T_460800) || ((int) global_args.BaudRate == BAUD_RATE_T_2000000))
            commSetBaudRate(&comm_settings_t, global_args.device_id, global_args.BaudRate);
        else

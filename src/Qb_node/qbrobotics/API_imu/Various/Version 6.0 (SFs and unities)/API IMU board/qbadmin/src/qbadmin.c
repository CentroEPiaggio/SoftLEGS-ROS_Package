...
static const struct option longOpts[] = {
   ...
   // { "set_baudrate", required_argument, NULL, 'R'},
	...
	{"get_imus_sfs", no_argument, NULL, 'S'},
...
struct global_args {
	...
	int flag_get_n_imu;				///< Imu board -N option
	int flag_get_mag_param;			///< Imu board -M option
	int flag_get_imu_readings;		///< Imu board -Q option
	int flag_set_imu_mode;			///< Imu board -Y option
	int flag_get_imus_mode;			///< Imu board -X option
	int flag_get_imus_sfs;			///< Imu board -S option
...
int main (int argc, char **argv)
{
	...
	global_args.n_imu					= 0;
	global_args.flag_get_imus_sfs		= 0;
    ...
    while ((option = getopt_long( argc, argv, optString, longOpts, &longIndex )) != -1)
    {
        switch (option)
        {
			...
			case 'M':
				...
				global_args.imus_magcal = (float*) calloc(4*17, sizeof(float));
				break;
			case 'Q':
				global_args.flag_get_imu_readings = 1;
				break;
			case 'Y':
				//qbadmin -Y id-acc_flag,gyro_flag,mag_flag
			...
			case 'X':
				...
				break;		
			case 'S':
				global_args.flag_get_imus_sfs = 1;
				break;
            case 'h':
            case '?':
            default:
...
//===============================================================     input file
    ...
    if(global_args.flag_get_mag_param)
    {
		...
		global_args.imus_magcal = (float *) calloc(global_args.n_imu, 4*sizeof(float));
        ...
        for (i = 0; i < global_args.n_imu; i++) {
			...
			printf("%d\t%f\t%f\t%f\n", (uint8_t)global_args.imus_magcal[4*i], global_args.imus_magcal[4*i + 1], global_args.imus_magcal[4*i + 2], global_args.imus_magcal[4*i + 3]);
        }
        printf("\n");
    ...
    if(global_args.flag_get_imu_readings)
    {
		...
		// Retrieve information on which sensors are read by each connected IMU
		...
		global_args.imus_sfs = (float *) calloc(NUM_SF_PARAMS, sizeof(float));
		commGetImusSFS(&comm_settings_1, global_args.device_id,
                    global_args.imus_sfs);
		...
		while(1){
			commGetImuReadings(&comm_settings_1, global_args.device_id, global_args.imu_table, global_args.imus_magcal, global_args.imus_sfs, global_args.n_imu, imu_values);
			
			printf("IMU: %d\n", global_args.imu_table[2*i + 0]);
			
			for (i = 0; i < global_args.n_imu; i++) {
				if (global_args.imu_table[2*i + 1] & 0x04){
					printf("Accelerometer\n");
					printf("%f, %f, %f\n", imu_values[3*3*i], imu_values[3*3*i+1], imu_values[3*3*i+2]);
				}
				if (global_args.imu_table[2*i + 1] & 0x02){
					printf("Gyroscope\n");
					printf("%f, %f, %f\n", imu_values[3*3*i+3], imu_values[3*3*i+4], imu_values[3*3*i+5]);
				}
				if (global_args.imu_table[2*i + 1] & 0x01){
					printf("Magnetometer\n");
    ...
    if(global_args.flag_get_imus_sfs)
    {
		
		uint8_t NUM_VALUES = 6;
		
		global_args.imus_sfs = (float *) calloc(NUM_VALUES, sizeof(float));
		
		commGetImusSFS(&comm_settings_1, global_args.device_id,
                    global_args.imus_sfs);
					
		printf("Accelerometers conversion factor: %f\n", global_args.imus_sfs[0]);
		printf("Gyroscopes conversion factor: %f\n", global_args.imus_sfs[1]);
		printf("Magnetometers conversion factor: %f\n", global_args.imus_sfs[2]);
		printf("\n");
		printf("Accelerometers unity: ");
		switch( (uint8_t)global_args.imus_sfs[3] ){
			case 0:
				printf("G");	break;
			case 1:
				printf("m/(s^2)");	break;
		...
		printf("\n");
		printf("Gyroscopes unity: ");
		switch( (uint8_t)global_args.imus_sfs[4] ){
			case 0:
				printf("deg/s");	break;
			case 1:
				printf("rad/s");	break;
		...
		printf("\n");
		printf("Magnetometers unity: ");
		switch( (uint8_t)global_args.imus_sfs[5] ){
			case 0:
				printf("uT");	break;
			default:
				break;
...
void display_usage( void )
{
    ...
    puts(" -Y, --set_imu_mode               Set sensors to read for each IMU [usage: qbadmin -Y id-acc_flag,gyro_flag,mag_flag]");
    puts(" -X, --get_imus_mode              Retrieve a summary of what we are reading from all imus");
    puts(" -S, --get_imus_sfs               Retrieve a summary of ACC, GYRO and MAG scale factors and unities");
	puts("");
    puts("--------------------------------------------------------------------------------");
    puts("Examples:");
    puts("");
